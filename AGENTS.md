...
> Este arquivo é **autossuficiente**: traz o contexto do caso **e** o protocolo de colaboração completo (embutido abaixo). Quem clonar este repositório tem tudo localmente — não é preciso buscar nada remoto.

## Contexto do problema
Projetar, de forma colaborativa (humanos + agentes), uma infraestrutura de **inferência** para o CIn-UFPE (complementando o cluster *batch* "Apuana"), incluindo prover um **LLM para a comunidade**. Entregável: **caminhos de decisão** com estimativas de custo/energia e opções de aquisição de GPUs, para a diretoria do CIn.
...
- `criteria.yaml` — critérios (QOC); `metrics.yaml` — fórmulas das estimativas; `assumptions.yaml` — premissas.
- `arguments.yaml` — argumentos pró/contra (IBIS + Dung).
- `cycles/NNN-slug.md` — registro de ciclo (front-matter YAML obrigatório).

## Antes de abrir o PR (validar localmente)
O motor vem da plataforma idea-waddle (clone-a ao lado):
```bash
git clone https://github.com/filipecalegario/idea-waddle ../idea-waddle
IW_CASE="$PWD" python ../idea-waddle/engine/lint.py
```
A CI também valida e checa se este protocolo embutido está em dia com a fonte (`PROTOCOL.md` da idea-waddle).
